...
<script>
import { timeDetia } from "../../common/timeUtil";
...
export default {
  ...
  methods: {
    ...
    timeDetia(date) {
      return timeDetia(date);
    },
    sendMsg () {
      if (this.chatm.length > 0 && this.fid == "") {
        let onemsg = {
          neir: this.chatm,
          img: this.madeUrl(this.img),
          time: new Date(),
          id: 2,
        };
        let data = {
    ...
    myself () {
      //获取即时信息渲染
      this.socket.on("myself", (name, member) => {
        //插入欢迎信息
        // this.uid = id;
  ...
  onLoad(e) {
    this.name = e.name;
    this.img = e.img;
    this.join(this.name, this.madeUrl(this.img));
    this.welcome();
    this.myself();
    this.receiveMsg();
    this.getmsg();
